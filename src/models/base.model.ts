...
export class BaseModel<T extends AnyObject> implements IBaseModel<T, PoolClient> {
  ...
  findLike(id: Partial<T>): Promise<T[]> {
  ...
  update(id: PrimaryKey, newValue: Partial<T>, tx?: PoolClient): Promise<T> {
  ...
  async findOne(id: PrimaryKey | Partial<T>, options: FindOptions<T, PoolClient> = {}): Promise<T> {
  ...
  async exist(id: PrimaryKey | Partial<T>, tx?: PoolClient): Promise<boolean> {
